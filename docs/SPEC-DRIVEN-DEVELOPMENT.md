...
2. **Product behavior** — what **SerialNumberBundle** guarantees to applications that integrate it (see [`USAGE.md`](USAGE.md), [`CONFIGURATION.md`](CONFIGURATION.md), [`INSTALLATION.md`](INSTALLATION.md)). **PHPUnit** and **PHPStan** enforce contracts in CI where applicable.
...
| Area | Responsibility |
| --- | --- |
| `SerialNumberGenerator` | Substitute `{context}` keys and `{id}` with optional zero-padding (max 32). |
